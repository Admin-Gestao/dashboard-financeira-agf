...
// Normalização das Categorias -> chaves do front
function normalizeCategoria(input: any): string {
  ...
  if (s.includes("telef") || s.includes("vivo") || s.includes("claro")) return "telefone";
  if (s.includes("veic") || s.includes("uber") || s.includes("motoboy") || s.includes("pedagio") || s.includes("estacionamento") || s.includes("posto") || s.includes("sem parar")) return "veiculos";
  if (s.includes("impost") || s.includes("iss") || s.includes("pis") || s.includes("cofins") || s.includes("irrf")) return "impostos";
  if (s.includes("folha") || s.includes("pgto") || s.includes("pagament")) return "folha_pagamento";
  if (s.includes("servic") || s.includes("material") || s.includes("manut") || s.includes("loca")) return "pitney"; // mapeia materiais/serviços Pitney
...
export async function GET(req: Request) {
  try {
    ...
    // 2) Lançamento Mensal (receita + total_despesa) da empresa
    ...
    for (const lm of lmRes.response.results) {
      ...
      if (lm._id && ano && mes) lmIndex.set(lm._id, { ano, mes, agfId, agfNome });
    }
    // **SEM usar spread para evitar erro de build no Vercel**
    ...
    // 4) SubContas (despesas detalhadas por categoria) — campo "LançamentoMesnal" + variações
    ...
    const dados: Record<number, Record<number, Record<string, Bucket>>> = {};
    ...
    // 5.1) Receitas & despesas totais (LM)
    for (const lm of lmRes.response.results) {
    ...
    // 5.3) Despesas por categoria (SubConta) — compatibilidade de campos
    for (const sc of scRes.response.results) {
      ...
      const catRaw = (() => {
        ...
        if (typeof c === "object")
          return (c as any).Categoria || (c as any).Nome || (c as any).name || "";
        return "";
      })();
    ...
    // 5.4) Se houver subcontas, a soma das categorias passa a ser o total_despesa
    for (const anoStr of Object.keys(dados)) {
      ...
      for (const mesStr of Object.keys(dados[ano])) {
        ...
        for (const agfNome of Object.keys(dados[ano][mes])) {
          ...
          const somaCategorias = Object.values(d.despesas).reduce(
            (a, b) => a + Number(b || 0),
            0
          );
          if (somaCategorias > 0) d.despesa_total = somaCategorias;
          else d.despesa_total = Number(d.despesa_total || 0);
        }
      }
    }
