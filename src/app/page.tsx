...
// --- DADOS MOCKADOS V4.0 (Com variação de Mês/Ano para teste de filtros) ---
  ...
  for (const ano of anos) {
    ...
    for (const mes of meses) {
      ...
      for (const agf of agfs) {
        ...
        const variacao = (ano - 2023) * 0.1 + (mes - 1) * 0.02 + (Math.random() - 0.5) * 0.1;
...
const agfList = [{ id: 'cl', nome: 'Campo Limpo' }, { id: 'rp', nome: 'Republica' }, { id: 'sj', nome: 'São Jorge' }, { id: 'jm', nome: 'Jd. Marajoara' }];
...
const ChartContainer = ({ title, children, className = "" }: { title: string, children: ReactElement, className?: string }) => (
  <div className={`bg-card p-4 rounded-lg flex flex-col ${className}`}>
    ...
    <div className="flex-grow h-full w-full"><ResponsiveContainer width="100%" height="100%">{children}</ResponsiveContainer></div>
  </div>
);
  ...
  if (active && payload && payload.length) {
    return (
      <div className="bg-background-end p-2 border border-primary/50 rounded-md text-sm">
        ...
        {payload.map((pld: any, index: number) => ( <p key={index} style={{ color: pld.fill || pld.stroke }}> {`${pld.name}: ${formatter(pld.value)}`} </p> ))}
      </div>
    );
  }
  return null;
    ...
    return (
        <div className="relative" ref={ref}>
            <button onClick={() => setIsOpen(!isOpen)} className="bg-card border border-primary/50 text-white p-2 rounded-md focus:ring-2 focus:ring-primary w-full flex justify-between items-center">
                <span>{name} ({selected.length || 'Todos'})</span>
                <ChevronDown size={16} />
            </button>
            {isOpen && (
...
export default function DashboardPage() {
  ...
  const dadosProcessados = useMemo(() => {
    const agfsParaProcessar = agfsSelecionadas.length > 0 ? agfsSelecionadas : mockApiData.agfs.map(a => a.nome);
    const anosParaProcessar = anosSelecionados.length > 0 ? anosSelecionados : anoList;
    const mesesParaProcessar = mesesSelecionados.length > 0 ? mesesSelecionados : mesList;

    const totaisPorAgf: any[] = [];
    for (const agfNome of agfsParaProcessar) {
        let totalReceita = 0, totalObjetos = 0;
        const totalDespesasPorCategoria: {[key: string]: number} = {};
        ...
        const despesaTotal = Object.values(totalDespesasPorCategoria).reduce((a, b) => a + b, 0);
        const resultado = totalReceita - despesaTotal;
        const margemLucro = totalReceita > 0 ? (resultado / totalReceita) * 100 : 0;
        const despesaSimulada = Object.entries(totalDespesasPorCategoria).filter(([key]) => !categoriasExcluidas.includes(key)).reduce((acc, [, val]) => acc + val, 0);
        ...
        totaisPorAgf.push({ nome: agfNome, receita: totalReceita, despesaTotal, resultado, margemLucro, objetos: totalObjetos, despesasDetalhadas: totalDespesasPorCategoria, margemLucroReal: margemLucro, ganhoMargem: ganhoMargem > 0 ? ganhoMargem : 0 });
    }

    const totaisGerais = { receita: totaisPorAgf.reduce((a, b) => a + b.receita, 0), despesa: totaisPorAgf.reduce((a, b) => a + b.despesaTotal, 0), resultado: totaisPorAgf.reduce((a, b) => a + b.resultado, 0), objetos: totaisPorAgf.reduce((a, b) => a + b.objetos, 0) };
    ...
    <div className="p-4 md:p-8">
      <main className="max-w-7xl mx-auto flex flex-col gap-8">
        ...
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <ChartContainer title="Comparativo de Receita" className="h-[280px]"><BarChart data={dadosProcessados.totaisPorAgf} margin={{ top: 20, right: 20, left: -20, bottom: 5 }}><CartesianGrid strokeDasharray="3 3" stroke="rgba(233, 242, 255, 0.1)" /><XAxis dataKey="nome" tick={{ fill: '#E9F2FF', opacity: 0.7, fontSize: 12 }} /><YAxis hide={true} /><Tooltip content={<CustomTooltip formatter={currencyFormatter} />} cursor={{ fill: 'rgba(255, 255, 255, 0.1)' }} /><Bar dataKey="receita" fill={CORES.receita} name="Receita"><LabelList dataKey="receita" position="top" formatter={compactNumberFormatter} style={{ fill: '#E9F2FF', fontSize: 12 }} /></Bar></BarChart></ChartContainer>
          <ChartContainer title="Comparativo de Despesa" className="h-[280px]"><BarChart data={dadosProcessados.totaisPorAgf} margin={{ top: 20, right: 20, left: -20, bottom: 5 }}><CartesianGrid strokeDasharray="3 3" stroke="rgba(233, 242, 255, 0.1)" /><XAxis dataKey="nome" tick={{ fill: '#E9F2FF', opacity: 0.7, fontSize: 12 }} /><YAxis hide={true} /><Tooltip content={<CustomTooltip formatter={currencyFormatter} />} cursor={{ fill: 'rgba(255, 255, 255, 0.1)' }} /><Bar dataKey="despesaTotal" fill={CORES.despesa} name="Despesa"><LabelList dataKey="despesaTotal" position="top" formatter={compactNumberFormatter} style={{ fill: '#E9F2FF', fontSize: 12 }} /></Bar></BarChart></ChartContainer>
          <ChartContainer title="Comparativo de Resultado" className="h-[280px]"><BarChart data={dadosProcessados.totaisPorAgf} margin={{ top: 20, right: 20, left: -20, bottom: 5 }}><CartesianGrid strokeDasharray="3 3" stroke="rgba(233, 242, 255, 0.1)" /><XAxis dataKey="nome" tick={{ fill: '#E9F2FF', opacity: 0.7, fontSize: 12 }} /><YAxis hide={true} /><Tooltip content={<CustomTooltip formatter={currencyFormatter} />} cursor={{ fill: 'rgba(255, 255, 255, 0.1)' }} /><Bar dataKey="resultado" fill={CORES.resultado} name="Resultado"><LabelList dataKey="resultado" position="top" formatter={compactNumberFormatter} style={{ fill: '#E9F2FF', fontSize: 12 }} /></Bar></BarChart></ChartContainer>
          <ChartContainer title="Comparativo de Margem de Lucro (%)" className="h-[280px]"><BarChart data={dadosProcessados.totaisPorAgf} margin={{ top: 20, right: 20, left: -20, bottom: 5 }}><CartesianGrid strokeDasharray="3 3" stroke="rgba(233, 242, 255, 0.1)" /><XAxis dataKey="nome" tick={{ fill: '#E9F2FF', opacity: 0.7, fontSize: 12 }} /><YAxis hide={true} /><Tooltip content={<CustomTooltip formatter={percentFormatter} />} cursor={{ fill: 'rgba(255, 255, 255, 0.1)' }} /><Bar dataKey="margemLucro" fill={CORES.margem} name="Margem"><LabelList dataKey="margemLucro" position="top" formatter={percentFormatter} style={{ fill: '#E9F2FF', fontSize: 12 }} /></Bar></BarChart></ChartContainer>
        </section>
        
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <ChartContainer title="Folha de Pagamento" className="h-[350px]"><BarChart data={dadosProcessados.totaisPorAgf} margin={{ top: 20, right: 20, left: -10, bottom: 5 }}><CartesianGrid strokeDasharray="3 3" stroke="rgba(233, 242, 255, 0.1)" /><XAxis dataKey="nome" tick={{ fill: '#E9F2FF', opacity: 0.7, fontSize: 12 }} /><YAxis tickFormatter={compactNumberFormatter} tick={{ fill: '#E9F2FF', opacity: 0.7, fontSize: 12 }} /><Tooltip content={<CustomTooltip formatter={currencyFormatter} />} cursor={{ fill: 'rgba(255, 255, 255, 0.1)' }} /><Bar dataKey="despesasDetalhadas.folha_pagamento" fill="#4472CA" name="Folha de Pagamento"><LabelList dataKey="despesasDetalhadas.folha_pagamento" position="top" formatter={compactNumberFormatter} style={{ fill: '#E9F2FF', fontSize: 12 }} /></Bar></BarChart></ChartContainer>
            <ChartContainer title="Total Gasto em Veículos por AGF" className="h-[350px]"><PieChart><Tooltip formatter={currencyFormatter} /><Legend wrapperStyle={{fontSize: "12px", opacity: 0.8}} /><Pie data={dadosProcessados.totaisPorAgf} dataKey="despesasDetalhadas.veiculos" nameKey="nome" cx="50%" cy="50%" outerRadius={100} labelLine={false} label={({ cx, cy, midAngle, innerRadius, outerRadius, percent, payload }) => { const radius = innerRadius + (outerRadius - innerRadius) * 1.2; const x = cx + radius * Math.cos(-midAngle * (Math.PI / 180)); const y = cy + radius * Math.sin(-midAngle * (Math.PI / 180)); return (<text x={x} y={y} fill="white" textAnchor={x > cx ? 'start' : 'end'} dominantBaseline="central" fontSize={12}>{compactNumberFormatter((payload as any).despesasDetalhadas.veiculos)}</text>);}}>{dadosProcessados.totaisPorAgf.map((_, index) => (<Cell key={`cell-${index}`} fill={['#F2935C', '#BF6550', '#4472CA', '#48DB8A'][index % 4]} />))}</Pie></PieChart></ChartContainer>
        </section>
        ...
        <section className="bg-card p-4 rounded-lg">
            ...
            <div className="mb-4"><p className="text-sm text-text/80 mb-2">Selecione despesas para excluir do cálculo:</p><div className="flex flex-wrap gap-2">{mockApiData.categoriasDespesa.map(cat => (<button key={cat} onClick={() => handleMultiSelect(setCategoriasExcluidas, cat)} className={`px-3 py-1 text-xs rounded-full transition-colors capitalize ${categoriasExcluidas.includes(cat) ? 'bg-primary text-white' : 'bg-gray-600/50 text-text/80'}`}>{cat.replace('_', ' ')}</button>))}</div></div>
            <ChartContainer title="" className="h-[300px]"><BarChart data={dadosProcessados.totaisPorAgf} layout="vertical" margin={{ top: 5, right: 30, left: 10, bottom: 5 }}><CartesianGrid strokeDasharray="3 3" stroke="rgba(233, 242, 255, 0.1)" /><XAxis type="number" tickFormatter={percentFormatter} tick={{ fill: '#E9F2FF', opacity: 0.7, fontSize: 12 }} /><YAxis type="category" dataKey="nome" stroke="#E9F2FF" tick={{ fill: '#E9F2FF', opacity: 0.7, fontSize: 12 }} width={80} /><Tooltip content={<CustomTooltip formatter={percentFormatter} />} cursor={{ fill: 'rgba(255, 255, 255, 0.1)' }} /><Legend wrapperStyle={{fontSize: "12px", opacity: 0.8}} /><Bar dataKey="margemLucroReal" stackId="a" fill={CORES.simulacaoReal} name="Margem Real"><LabelList dataKey="margemLucroReal" position="center" formatter={percentFormatter} style={{ fill: '#E9F2FF', fontSize: 12 }} /></Bar><Bar dataKey="ganhoMargem" stackId="a" fill={CORES.simulacaoGanho} name="Ganho de Margem"><LabelList dataKey="ganhoMargem" position="center" formatter={percentFormatter} style={{ fill: '#010326', fontSize: 12, fontWeight: 'bold' }} /></Bar></BarChart></ChartContainer>
        </section>
      </main>
    </div>
  );
