...
export default function DashboardPage() {
  ...
  const sourceCategorias = apiData?.categoriasDespesa ?? mockApiData.categoriasDespesa;
  const sourceDados = apiData?.dados ?? mockApiData.dados;

  const dadosProcessados = useMemo(() => {
    const idsAgf = agfsSelecionadas.length > 0 ? agfsSelecionadas : sourceAgfs.map((a) => a.id);
  ...
  }, [agfsSelecionadas, mesesSelecionados, anosSelecionados, categoriasExcluidas, sourceAgfs, sourceCategorias, sourceDados]);
        ...
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <ChartContainer title="Folha de Pagamento" className="h-[350px]">
            <BarChart data={dadosProcessados.totaisPorAgf} margin={{ top: 20, right: 20, left: -10, bottom: 5 }}>
              ...
              <Tooltip content={<CustomTooltip formatter={currencyFormatte
